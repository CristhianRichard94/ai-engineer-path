// Backend API client for the Flask indexing/prompt service.
 ...
 * Throws ApiError on 400/5xx or network failure.
 */
export async function postIndex(url: string): Promise<IndexPostResponse> {
  let response: Response;
  try {
    response = await fetch(`${getApiBaseUrl()}/api/index`, {
      method: "POST",
...
export async function getIndexStatus(
  ...
  let response: Response;
  try {
    response = await fetch(`${getApiBaseUrl()}/api/index/${encodeURIComponent(jobId)}`);
  } catch {
 ...
 * - Non-SSE synchronous 400 JSON error (validation failure / repo not indexed).
...
export async function streamPrompt(
  ...
  let response: Response;
  try {
    response = await fetch(`${getApiBaseUrl()}/api/prompt/stream`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
